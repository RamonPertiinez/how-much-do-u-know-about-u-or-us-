(() => {
  ...
  const els = {
    ...
    user: qs("#username"),
    pass: qs("#password"),
    loginMsg: qs("#loginMsg"),

    progressFill: qs("#progressFill"),
    scoreNum: qs("#scoreNum"),
    scoreGoal: qs("#scoreGoal"),

    grid: qs("#challengeGrid"),
    btnShuffle: qs("#btnShuffle"),
    btnReset: qs("#btnReset"),
    easterReset: qs("#easterReset"),

    btnBack: qs("#btnBack"),
    chTitle: qs("#challengeTitle"),
    chIntro: qs("#challengeIntro"),
    chContent: qs("#challengeContent"),
    btnSubmit: qs("#btnSubmit"),
    feedback: qs("#feedback"),

    finalHeading: qs("#finalHeading"),
    finalSub: qs("#finalSubheading"),
  ...
  // ---------- Defaults (per si no tens /data encara) ----------
  const DEFAULT_CONFIG = {
    ...
    final: {
      ...
      body:
        "Al gener, porta la teva roba més elegant. Tinc reservat un lloc especial per sopar junts a Roma. 💛",
      audio: "" // deixa buit si no tens arxiu
    },
    gateRiddle: {
      // Si NO poses 'text' aquí, es mantindrà l'HTML que has escrit a index.html
  ...
  const DEFAULT_CHALLENGES = [
    {
      id: "c1",
      title: "On va començar tot?",
      intro: "Recordes el primer lloc on vam dormir junts?",
      type: "text",
      emoji: "💭",
      answer: "poblenou",
      accept: ["poble nou", "poblenou"],
      score: 1
    },
    {
      id: "c2",
      title: "Endevina la cançó",
      intro: "Escolta el fragment i escriu el títol (pots provar ‘yellow’).",
      type: "audio",
      emoji: "🎵",
      src: "assets/audio/intro.mp3",
      accept: ["yellow", "yellow coldplay"],
      score: 1
    },
    {
      id: "c3",
      title: "Quina foto és?",
      intro: "Una pista borrosa d'un lloc nostre",
      type: "image-guess",
      emoji: "🖼️",
      src: "assets/images/sample.jpg",
      accept: ["roma", "colosseu", "colosseum"],
      score: 1
    },
    {
      id: "c4",
      title: "El detall graciós",
      intro: "Quina és la meva mania més estranya segons tu?",
      type: "text",
      emoji: "😜",
      answer: "endreçar els coixins",
      accept: ["endreçar coixins", "endreçar els coixins", "ordenar els coixins"],
      score: 1
    },
    {
      id: "c5",
      title: "Records en ordre",
      intro: "Ordena on vam viatjar (més antic → més recent)",
      type: "choice",
      emoji: "🧭",
      options: [
        "Roma → Lisboa → Menorca",
  ...
  function save() {
    localStorage.setItem(
      storageKey,
      JSON.stringify({ solved: state.solved, score: state.score })
    );
  }
  function load() {
    try {
  ...
  async function boot() {
    ...
    if (cfg.final) {
      if (cfg.final.heading) els.finalHeading.textContent = cfg.final.heading;
      if (cfg.final.subheading) els.finalSub.textContent = cfg.final.subheading;
      if (cfg.final.body) els.finalBody.textContent = cfg.final.body;
      if (cfg.final.audio) {
        const s = document.createElement("source");
        s.src = cfg.final.audio;
        s.type = "audio/mpeg";
        els.finalAudio.appendChild(s);
      }
    }

    // Inicialitza la UI del riddle (respecta l'HTML del index si no hi ha text al config)
    setupRiddleUI();

    // Login
    els.loginForm.addEventListener("submit", (e) => {
      e.preventDefault();
      const u = norm(els.user.value);
      ...
      if (u === norm(cfg.auth.username) && p === cfg.auth.password) {
        ...
        els.loginMsg.textContent = "Ups! Credencials incorrectes";
    ...
    // Easter egg: long-press 🎁 to unhide reset
    let t;
    els.easterReset?.addEventListener(
      "pointerdown",
      () => (t = setTimeout(() => (els.btnReset.hidden = false), 800))
    );
    ["pointerup", "pointerleave", "pointercancel"].forEach((ev) =>
      els.easterReset?.addEventListener(ev, () => clearTimeout(t))
    );
  ...
  async function fetchJSON(path, fallback) {
    try {
      ...
      console.warn(`[app] No s'ha trobat ${path}. Carregant per defecte.`);
      return structuredClone(fallback);
    }
  }

  // ---------- Riddle (porta d'entrada) ----------
  function setupRiddleUI(){
    ...
    // Si hi ha 'text' al config, l'escrivim com a text pla; si no, respectem l'HTML del index.html
  ...
  function updateProgress() {
    ...
    if (state.score >= goal) {
      ...
      setTimeout(() => {
        swap(VIEWS.final);
        try {
          els.finalAudio && els.finalAudio.play().catch(() => {});
        } catch (e) {}
      }, 600);
    }
  }
  ...
  function celebrate() {
    ...
    for (let i = 0; i < 80; i++) {
      ...
      requestAnimationFrame(() => {
        s.style.top = "110vh";
        s.style.transform = `translateY(100vh) rotate(${Math.random() * 720}deg)`;
      });
  ...
  function renderGrid(shuffleNow = false) {
    let list = state.challenges.slice();
    if (shuffleNow) list = shuffle(list);
    els.grid.innerHTML = "";
    list.forEach((ch) => {
  ...
  function openChallenge(id) {
    ...
    // Aplica la teva lletra a títols especials si 'handTitle' és true
    ...
    if (ch.type === "choice") {
      ...
      inp.type = "text";
      inp.placeholder = ch.placeholder || "Escriu la resposta";
      inp.id = "textAnswer";
      c.appendChild(inp);
    } else if (ch.type === "audio") {
      const au = document.createElement("audio");
      au.controls = true;
      au.className = "audio-ctrl";
      au.src = ch.src || "";
      c.appendChild(au);
      const inp = document.createElement("input");
      inp.type = "text";
      inp.placeholder = "Títol/cançó";
      inp.id = "textAnswer";
      c.appendChild(inp);
    } else if (ch.type === "image-guess") {
  ...
  function onSubmit() {
    ...
    } else if (ch.type === "text" || ch.type === "audio" || ch.type === "image-guess") {
      const el = qs("#textAnswer");
      const val = norm(el?.value);
      if (Array.isArray(ch.accept)) ok = ch.accept.map(norm).includes(val);
      else ok = norm(ch.answer) === val;
    }

    if (ok) {
      if (!state.solved[ch.id]) {
        state.solved[ch.id] = true;
        state.score += ch.score || 1;
        save();
      }
      els.feedback.textContent = "Correcte! 🥳";
      ...
      els.feedback.textContent = "Mmm… intenta-ho una altra vegada!";
      els.feedback.className = "msg err";
      if (ch.type === "image-guess") {
        const img = els.chContent.querySelector("img");
        if (img) img.style.filter = "blur(3px)"; // lleu pista després d'un error
      }
    }
  }
})();
