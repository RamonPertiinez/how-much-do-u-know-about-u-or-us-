(() => {
  ...
  const DEFAULT_CHALLENGES = [
    ...
    { id:"c5", title:"Records en ordre", intro:"Ordena on vam viatjar (més antic → més recent)", type:"choice", emoji:"🧭", options:["Roma → Lisboa → Menorca","Lisboa → Menorca → Roma","Menorca → Roma → Lisboa"], answerIndex:2, score:1 }
  ];

  function save(){ localStorage.setItem(storageKey, JSON.stringify({solved:state.solved, score:state.score})); }
  function load(){ try{ const d=JSON.parse(localStorage.getItem(storageKey)); if(d){ state.solved=d.solved||{}; state.score=d.score||0; } }catch(e){} }
  function swap(v){ Object.values(VIEWS).forEach(x=>x.classList.remove("active")); v.classList.add("active"); }
  ...
  async function fetchJSON(path, fallback){
    try{ const res=await fetch(path,{cache:"no-cache"}); if(!res.ok) throw new Error("not ok"); return await res.json(); }
    catch(e){ console.warn(`[app] No s'ha trobat ${path}. Carregant per defecte.`); return structuredClone(fallback); }
  }
  ...
  function setupRiddleUI(){
    ...
    // Si hi ha text al config, l'escrivim; si no, respectem l'HTML posat a index.html
  ...
  function updateProgress(){
    ...
    if(state.score>=goal){
      celebrate();
      setTimeout(()=>{ swap(VIEWS.final); try{ els.finalAudio && els.finalAudio.play().catch(()=>{});}catch(e){} }, 600);
    }
  }

  function celebrate(){
    const container = els.confetti; container.innerHTML='';
    for(let i=0;i<80;i++){
  ...
  function renderGrid(shuffleNow=false){
    ...
    els.grid.innerHTML=''; list.forEach(ch=>{
      const t=document.createElement('button'); t.className='tile'; t.dataset.id=ch.id; t.setAttribute('role','listitem');
  ...
  function openChallenge(id){
    ...
    if(ch.type==='choice'){
      ch.options.forEach((opt,i)=>{ const lbl=document.createElement('label'); lbl.className='option'; lbl.innerHTML=`<input type="radio" name="opt" value="${i}"> ${opt}`; c.appendChild(lbl); });
      ...
      const au=document.createElement('audio'); au.controls=true; au.className='audio-ctrl'; au.src=ch.src||''; c.appendChild(au);
      const inp=document.createElement('input'); inp.type='text'; inp.placeholder='Títol/cançó'; inp.id='textAnswer'; c.appendChild(inp);
    } else if (ch.type==='image-guess'){
      const img=document.createElement('img'); img.src=ch.src||''; img.alt='pista'; img.style.filter='blur(6px)'; c.appendChild(img);
      const inp=document.createElement('input'); inp.type='text'; inp.placeholder='On és això?'; inp.id='textAnswer'; c.appendChild(inp);
    } else { c.textContent='Tipus no implementat encara.'; }
  ...
  function onSubmit(){
    ...
    let ok=false;
    if(ch.type==='choice'){
      const sel=document.querySelector('input[name="opt"]:checked'); ok = sel && Number(sel.value)===Number(ch.answerIndex);
